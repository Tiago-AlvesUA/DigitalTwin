...
def stacked_bar(x, values, label):
    plt.bar(x, values[0], width=bar_width, color=colors[0], label="OBU → Broker → Agent" if label == "Average" else "")
    plt.bar(x, values[1], width=bar_width, bottom=values[0], color=colors[1], label="Collision Check" if label == "Average" else "")
    plt.bar(x, values[2], width=bar_width, bottom=values[0] + values[1], color=colors[2], label="Alert Generation → Ditto" if label == "Average" else "")
    plt.bar(x, values[3], width=bar_width, bottom=sum(values[:3]), color=colors[3], label="Ditto → OBU(WS Client)" if label == "Average" else "")
