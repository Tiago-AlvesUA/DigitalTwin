...
WS_RECEIVE_TIMES = "checkpoints/ws-receive-times.csv"
MAX_SAMPLES = 1000
delays = []
receive_times = []
sample_count = 0
...
def on_message(ws, message):
    global writer, logfile, sample_count, delays
    ...
    if "features/ModemStatus" in path:
        ...
        t_feature_update_rcv_mec_seconds = time.time()
        receive_times.append(t_feature_update_rcv_mec_seconds)

        t_feature_update_rcv_mec = round(t_feature_update_rcv_mec_seconds * 1000)

        t_modStatus_gen_obu = value.get("referenceTime", 0)
        #t_modStatus_gen_obu = t_modStatus_gen_obu_raw % 65536  # Reference time from the CAM, in milliseconds

        # # when t_feature_update_rcv_mec < t_modStatus_gen_obu, the delays might not be right and be too high
        ...
        if sample_count >= MAX_SAMPLES:
            ...
            pd.DataFrame(delays, columns=["delay_ms"]).to_csv(LATENCY_LOG, index=False)
            pd.DataFrame(receive_times, columns=["receive_time_unix"]).to_csv(WS_RECEIVE_TIMES, index=False)
            ws.close()
            sys.exit(0)      
