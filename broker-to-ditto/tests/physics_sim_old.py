...
def run(window, width, height):
    ...
    sender_body, sender_vehicle = create_vehicle(space, sender_position, 10, 1.8, 4.3, sender_heading, 2500*1e-2, color=(0, 0, 255, 255)) # blue
    receiver_body, receiver_vehicle = create_vehicle(space, receiver_position, 10, 1.8, 4.3, receiver_heading, 2500*1e-2, color=(255, 0, 0, 255)) # red
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                # TODO: check correct form of acceleration
                apply_force(sender_body, 161)
                apply_force(receiver_body, 161)
